# todo: try to implement dense_net by tensorflow 
# date: 2018-04-21 
...
def build_fc_densenet(inputs, n_classes, preset_model='FC-DenseNet56', n_filters_first_conv=48, 
	...
	with tf.variable_scope(scope, preset_model, [inputs]) as sc:
		...
		for i in range(n_pool):
			...
			# skip_connection_list.append(None) # For image compression 
			...
			while epoch < self.nb_epoch: 
				...
				while step < nb_step: 
					...
					_, loss, e_loss, r_loss, outputs, max_z, min_z = sess.run([self.opt, self.loss, self.entropy_loss, self.recon_loss, self.outputs, self.max_z, self.min_z], 
						feed_dict={self.inputs:real_batch})
					...
					if step % (nb_step // 2) == 1:
						real_batch = self.val_data.get_next_batch()
						...
						save_images_in_folder(outputs, self.output_dir + 'epoch_%d_step_%d_' % (epoch, step))
	...
	def test(self):
		...
		with tf.Session(config=config) as sess:
			...
			for idx in range(self.val_data.db_size):
				real_batch = self.train_data.get_next_batch(batch_size=1)
				loss, outputs = sess.run([self.loss, self.outputs], feed_dict={self.inputs:real_batch})
				mse, psnr = mse_tf(real_batch, outputs)
				writestr = 'image %d/%d loss: %f mse: %f psnr: %f' % (idx, self.val_data.db_size, loss, mse, psnr)
				print(writestr)
				save_dir = self.output_dir + '../test/%04d.png' % (idx)
				mysave(save_dir, outputs[0,:,:,:])
